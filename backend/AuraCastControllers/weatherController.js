...
// Replace hardcoded geocodeLocation with OpenStreetMap implementation
const geocodeLocation = async (location) => {
  try {
    ...
    // Use the autocomplete service to get coordinates
    const suggestions = await autocompleteService.getLocationSuggestions(location);
    
    if (suggestions.length === 0) {
      // Fallback to common cities if no results
      const fallbackSuggestions = autocompleteService.getFallbackSuggestions(location);
    ...
    console.log(`✅ Geocoded "${location}" to:`, bestMatch.display_name);
...
export { analyzeWeather, geocodeLocation };

...
const WeatherForm = ({ onAnalysis }) => {
  ...
  const eventTypes = [
    ...
    { id: 'vacation', name: 'Vacation', icon: '✈️', supportsRange: true },
    { id: 'hiking', name: 'Hiking', icon: '🥾', supportsRange: true },
    { id: 'fishing', name: 'Fishing', icon: '🎣', supportsRange: true },
    ...
    { id: 'gardening', name: 'Gardening', icon: '🌻', supportsRange: true },
    { id: 'corporate_event', name: 'Corporate Event', icon: '🏢', supportsRange: true },
    { id: 'other', name: 'Other', icon: '📅', supportsRange: true }
  ];
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-8 space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Get personalized weather insights for your special event</p>
      </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {eventTypes.map(event => (
              <button
                key={event.id}
                type="button"
                className={`p-3 rounded-lg border-2 transition-all duration-200 flex flex-col items-center justify-center gap-1 ${
                  eventType === event.id 
              ...
              >
                ...
                {event.supportsRange && (
                  <span className="text-[10px] text-blue-500 mt-1">Multi-day</span>
                )}
              </button>
            ))}
          </div>
        ...
        >
          {loading ? (
            <span className="flex items-center justify-center">
              ...
              {isDateRange ? 'Analyzing Date Range...' : 'Analyzing Weather...'}
            </span>
          ) : (
            `Check Weather Risk${isDateRange ? ' for Date Range' : ''}`
          )}
        </button>
      </form>
    </div>

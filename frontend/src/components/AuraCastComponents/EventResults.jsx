// frontend/src/components/AuraCastComponents/EventResults.jsx
          ...
          <div key={index} className="bg-white rounded-xl p-4 shadow-md border border-gray-100">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-orange-600">{day.avgMaxTemperature ? Math.round(day.avgMaxTemperature) : 'N/A'}°C</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Min Temp:</span>
                <span className="font-semibold text-blue-600">{day.avgMinTemperature ? Math.round(day.avgMinTemperature) : 'N/A'}°C</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Humidity:</span>
                <span className="font-semibold">{day.avgHumidity ? Math.round(day.avgHumidity) : 'N/A'}%</span>
              </div>
              <div className="flex justify-between">
...
const EventVenueRecommendations = ({ recommendations }) => {
...
const GuestComfortAnalysis = ({ weatherData }) => {
  const comfortLevels = [
    { level: 'Excellent', color: 'text-green-600', bg: 'bg-green-100', description: 'Perfect conditions for guests' },
    { level: 'Good', color: 'text-blue-600', bg: 'bg-blue-100', description: 'Comfortable with minor considerations' },
    { level: 'Fair', color: 'text-yellow-600', bg: 'bg-yellow-100', description: 'Some adjustments needed' },
    { level: 'Poor', color: 'text-red-600', bg: 'bg-red-100', description: 'Significant comfort concerns' }
  ];

  const getComfortLevel = () => {
    const { rainProbability, avgTemperature, extremeConditions } = weatherData;
    
    if (rainProbability > 50 || extremeConditions?.veryHot > 30 || extremeConditions?.veryCold > 20) {
      return comfortLevels[3]; // Poor
    } else if (rainProbability > 30 || extremeConditions?.veryHot > 15 || extremeConditions?.veryCold > 10) {
      return comfortLevels[2]; // Fair
    } else if (rainProbability > 15) {
      return comfortLevels[1]; // Good
    } else {
      return comfortLevels[0]; // Excellent
    }
  };
  ...
  return (
    <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 shadow-lg">
      ...
      <div className={`p-4 rounded-lg ${comfort.bg} ${comfort.color} text-center`}>
        <div className="text-2xl font-bold mb-2">{comfort.level}</div>
        <p className="text-sm">{comfort.description}</p>
      </div>
    </div>
  );
};
...
// Preparation Advice Component
const PreparationAdviceDisplay = ({ preparationAdvice }) => {
  if (!preparationAdvice || preparationAdvice.length === 0) return null;
...
const EventResults = ({ data }) => {
  ...
  const climateTrends = data.climateTrends;

  // Date range specific data
  const isDateRange = data.isDateRange || false;
  const dateRange = data.dateRange || 1;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in p-4">
      {/* Event Header - Enhanced with location and date info */}
      <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
        <div className="flex items-center justify-center mb-4">
          <span className="text-4xl mr-4">{currentEvent.icon}</span>
          <div>
            <h3 className="text-3xl font-bold text-gray-800 mb-2">Event Weather Analysis</h3>
      ...
      {/* Guest Comfort */}
      <GuestComfortAnalysis weatherData={{ rainProbability, avgTemperature, extremeConditions }} />
      ...
      {/* Climate Trends */}
      {climateTrends && (
        <ClimateTrendsDisplay climateTrends={climateTrends} />
      )}

      {/* Preparation Advice */}
      <PreparationAdviceDisplay preparationAdvice={preparationAdvice} />

      {/* Event-specific Recommendations */}
      <EventVenueRecommendations recommendations={recommendations} />

      {/* Data Source */}
      <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl p-6 shadow-lg">
        <div className="text-center">
          <h4 className="text-lg font-semibold text-gray-700 mb-4">Event Analysis Summary</h4>
...
//   // Extract all data with proper fallbacks (same as old component)
//   const basicWeather = data.basicWeather || data;
//   const rainProbability = basicWeather.rainProbability || 0;
//   const avgTemperature = basicWeather.avgTemperature;
//   const avgWindSpeed = basicWeather.avgWindSpeed;
